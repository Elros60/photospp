...
STDLIB = /opt/homebrew/Cellar/gcc/14.2.0/lib/gcc/14/libstdc++.dylib
...
all: include_dir lib_dir $(EVENT_RECORD_INTERFACE_DIR) $(FORTRAN_PHOTOS_INTERFACE_DIR) $(PHOTOS_C_INTERFACE_DIR) $(PHOTOS_C_DIR) $(UTILITIES_DIR)
	...
	$(LD) $(LDFLAGS) $(SOFLAGS) -o lib/$(LIB_PHOTOSPP_SO).$(LIB_VER) -Wl,-install_name,$(LIB_PHOTOSPP_SO) $(LIB_PHOTOSPP_OBJECTS) $(STDLIB)
	ar cr lib/$(LIB_PHOTOSPP_HEPEVT_A) $(LIB_PHOTOSPP_HEPEVT_OBJECTS)
	$(LD) $(LDFLAGS) $(SOFLAGS) -o lib/$(LIB_PHOTOSPP_HEPEVT_SO).$(LIB_VER) -Wl,-install_name,$(LIB_PHOTOSPP_HEPEVT_SO) $(LIB_PHOTOSPP_HEPEVT_OBJECTS) $(STDLIB)
...
ifneq ($(HEPMCLOCATION), )
	...
	$(LD) $(LDFLAGS) $(SOFLAGS) -o lib/$(LIB_PHOTOSPP_HEPMC_SO).$(LIB_VER) -Wl,-install_name,$(LIB_PHOTOSPP_HEPMC_SO) $(LIB_PHOTOSPP_HEPMC_OBJECTS) $(STDLIB)
...
ifneq ($(HEPMC3LOCATION), )
	...
	$(LD) $(LDFLAGS) $(SOFLAGS) -o lib/$(LIB_PHOTOSPP_HEPMC3_SO).$(LIB_VER) -Wl,-install_name,$(LIB_PHOTOSPP_HEPMC3_SO) $(LIB_PHOTOSPP_HEPMC3_OBJECTS) $(STDLIB)
